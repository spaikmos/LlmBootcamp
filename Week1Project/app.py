...
async def assess_message(message_history):
    ...
    alerts_str = json.dumps(parsed_record.get("Vocab", []), indent=4)
    knowledge_str = json.dumps(parsed_record.get("Topic", {}), indent=4)
    ...
    filled_prompt = ASSESSMENT_PROMPT.format(
        ...
        history=history_str,
        korean_vocab_level=alerts_str,
        conversation_topic_level=knowledge_str,
    ...
    parsed_record["Vocab"].extend(new_alerts)
    for update in knowledge_updates:
        date = update["date"]
        level = update["level"]
        parsed_record["Knowledge"][date] = level
